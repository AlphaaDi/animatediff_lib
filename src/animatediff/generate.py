...
def map_rgb_to_labels(rgb_mask):
    # Flatten the array and convert to a structured array so that each RGB value is a unique element
    flat_rgb = rgb_mask.reshape(-1, rgb_mask.shape[-1]).astype(np.uint8)
    flat_rgb.dtype = [('r', np.uint8), ('g', np.uint8), ('b', np.uint8)]
    
    # Find the unique colors (structured array elements)
    unique_colors, unique_labels = np.unique(flat_rgb, return_inverse=True)
    
    # Map back to the original image shape
    labeled_mask = unique_labels.reshape(rgb_mask.shape[:-1])
    return labeled_mask, unique_colors

# Function to filter masks by size
def filter_masks_by_size(labeled_mask, unique_colors, rgb_mask, size_threshold):
    # Create an empty array with the same shape as the labeled mask
    ...
    for label in np.unique(labeled_mask):
        ...
        binary_mask = (labeled_mask == label)
        
        # Count the number of pixels for the current label
        mask_size = np.sum(binary_mask)
        
        # If the size is greater than the threshold, add it to the filtered mask
        if mask_size > size_threshold:
            filtered_mask[binary_mask] = label
            
    # Convert labels back to RGB
    filtered_rgb_mask = np.zeros(rgb_mask.shape, dtype=np.uint8)
    for label, color in enumerate(unique_colors):
        filtered_rgb_mask[filtered_mask == label] = color.item()
...
def fill_holes_in_mask(rgb_mask, threshold=1000):
    ...
    # Threshold the image to create a binary mask where the non-zero regions are considered as mask
    _, binary_mask = cv2.threshold(gray_mask, 1, 255, cv2.THRESH_BINARY_INV)
    
    # Find contours and hierarchy which correspond to the holes
    contours, hierarchy = cv2.findContours(binary_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    
    # If there are no contours, just return the original mask
    if len(contours) == 0:
        return rgb_mask

    # Fill the holes (contours with a parent contour are considered holes)
    if hierarchy is not None:
        for i, contour in enumerate(contours):
            if cv2.contourArea(contour) > threshold:
                continue
            max_x_idx = contour[:,:,0].argmax()
            point = contour[max_x_idx, 0].copy()
            point[0] += 1
            point[1] += 1
            color = rgb_mask[point[1], point[0]]
            color = ( int (color [ 0 ]), int (color [ 1 ]), int (color [ 2 ])) 
            cv2.drawContours(rgb_mask, contours, i, tuple(color), -1)
    
    return rgb_mask


def create_index_to_color_mapping(index_mask, rgb_mask):
    unique_indexes = np.unique(index_mask)
    index_to_color = {}
    for index in unique_indexes:
        if index == 0:
            continue  # Assuming index 0 is the background and should be skipped
        mask = index_mask == index
        colors = rgb_mask[mask]
        if len(colors) > 0:
            index_to_color[index] = colors[0]  # Take the first color encountered
    return index_to_color

def apply_color_mapping(index_mask, index_to_color):
    # Create an empty RGB image with the same dimensions as the index mask
    rgb_output = np.zeros((*index_mask.shape, 3), dtype=np.uint8)
    
    # Map each index to its corresponding color
    for index, color in index_to_color.items():
        rgb_output[index_mask == index] = color
...
class SamSegPreProcessor:
    ...
    def process_first_mask(self, input_image):
        with torch.no_grad():
    ...
    def __call__(self, input_image, image_resolution=512, output_type="pil", **kwargs):
        input_image = np.array(input_image)
        if self.first_mask is None:
            self.first_mask = self.process_first_mask(input_image)
            self.first_frame = input_image
            map = self.first_mask
            ...
            size = self.xmem_size
            rgb_mask = cv2.resize(self.first_mask, (size,size))
            frames_res = [
                cv2.resize(self.first_frame, (size,size)), 
                cv2.resize(input_image, (size,size))
            ]
            with torch.no_grad():
                masks = self.xmem(
                    frames_res, 
            ...
            map = masks[1]
            index_to_color = create_index_to_color_mapping(masks[0], rgb_mask)
            map = apply_color_mapping(map, index_to_color)
...
def create_preprocessor_from_name(pre_type):
    ...
    elif pre_type == "sam_seg":
        return SamSegPreProcessor('/home/ishpuntov/code/animatediff-cli-prompt-travel/sam_hq_vit_l.pth', 'cuda')
    elif pre_type == "blur":
...
def create_pipeline(
    ...
    if model_config.path is not None:
        ...
        unet_missing, unet_unex = unet.load_state_dict(unet_state_dict, strict=False)
        # print('unet_missing, unet_unex', unet_missing, unet_unex)
        if len(unet_unex) > 0:
            raise ValueError(f"UNet has unexpected keys: {unet_unex}")
        tenc_missing, tenc_unex = text_encoder.load_state_dict(tenc_state_dict, strict=False)
        if len(tenc_missing) > 0:
            raise ValueError(f"TextEncoder has missing keys: {tenc_missing}")
        vae_missing, vae_unex = vae.load_state_dict(vae_state_dict, strict=False)
...
def ip_adapter_preprocess(
        ip_adapter_config_map: Dict[str, Dict[str, Any]] = None,
        width: int = 512,
        height: int = 512,
        duration: int = 16,
        out_dir: PathLike = ...,
    ...
    if ip_adapter_config_map:
        for adapter_type, config in ip_adapter_config_map.items():
            if config.get("enable", False):
                resized_to_square = config.get("resized_to_square", False)
                image_dir = data_dir.joinpath(config["input_image_dir"])
                imgs = sorted(glob.glob(os.path.join(image_dir, "[0-9]*.png"), recursive=False))

                if imgs:
                    prepare_ip_adapter()  # Assuming this function is defined
                    adapter_specific_map = {
                        "scale": config.get("scale"),
                        adapter_type: True,
                        "images": {}
                    }

                    for img_path in tqdm(imgs, desc=f"Preprocessing images ({adapter_type})"):
                        frame_no = int(Path(img_path).stem)
                        if frame_no < duration:
                            if resized_to_square:
                                adapter_specific_map["images"][frame_no] = get_resized_image(img_path, 256, 256)
                            else:
                                adapter_specific_map["images"][frame_no] = get_resized_image2(img_path, 256)
                            processed = True

                    ip_adapter_map[adapter_type] = adapter_specific_map

                    if config.get("save_input_image", False) and processed:
                        det_dir = Path(out_dir).joinpath(f"{0:02d}_{adapter_type}/")
                        det_dir.mkdir(parents=True, exist_ok=True)
                        for frame_no, image in tqdm(adapter_specific_map["images"].items(), desc=f"Saving Preprocessed images ({adapter_type})"):
                            save_path = det_dir.joinpath(f"{frame_no:08d}.png")
                            image.save(save_path)
    return ip_adapter_map if processed else None


def ip_adapter_preprocess_old(
