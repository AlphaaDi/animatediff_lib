...
class AnimationPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.ip_adapters = None
        if ip_adapter_map:
            self.ip_adapters = {}
            img_enc_path = "data/models/ip_adapter/models/image_encoder/"

            for adapter_type, adapter_config in ip_adapter_map.items():
                if adapter_type == "is_light":
                    self.ip_adapters[adapter_type] = IPAdapter(self, img_enc_path, "data/models/ip_adapter/models/ip-adapter_sd15_light.bin", device, 4)
                elif adapter_type == "is_plus_face":
                    self.ip_adapters[adapter_type] = IPAdapterPlus(self, img_enc_path, "data/models/ip_adapter/models/ip-adapter-plus-face_sd15.bin", device, 16)
                elif adapter_type == "is_plus":
                    self.ip_adapters[adapter_type] = IPAdapterPlus(self, img_enc_path, "data/models/ip_adapter/models/ip-adapter-plus_sd15.bin", device, 16)
                else:
                    self.ip_adapters[adapter_type] = IPAdapter(self, img_enc_path, "data/models/ip_adapter/models/ip-adapter_sd15.bin", device, 4)

                self.ip_adapters[adapter_type].set_scale(adapter_config["scale"])
        ...
        if self.ip_adapters:
            for ip_adapter in self.ip_adapters.values():
                ip_adapter.set_text_length(positive[0].shape[1])
        ...
        if self.ip_adapters:
            ...
            first_last_key_frames = {}

            for adapter_type, adapter in self.ip_adapters.items():
                ip_im_map = ip_adapter_map[adapter_type]["images"]
                ip_im_map = dict(sorted(ip_im_map.items()))

                if ip_im_map:
                    ip_im_list = [ip_im_map[key_frame] for key_frame in ip_im_map.keys()]

                    positive, negative = adapter.get_image_embeds(ip_im_list)
                    ...
                    im_prompt_embeds_map[adapter_type] = {}
                    for i, key_frame in enumerate(ip_im_map):
                        if do_classifier_free_guidance:
                            im_prompt_embeds_map[adapter_type][key_frame] = torch.cat([negative[i], positive[i]])
                        else:
                            im_prompt_embeds_map[adapter_type][key_frame] = positive[i]

                    # Store first and last key frames for each enabled adapter
                    if ip_im_map:
                        first_last_key_frames[adapter_type] = {
                            'first': list(ip_im_map.keys())[0],
                            'last': list(ip_im_map.keys())[-1]
                        }
            
        
        def get_current_prompt_embeds_from_image(center_frame: int, video_length: int):
            all_embeddings = []

            for adapter_type, embeddings_map in im_prompt_embeds_map.items():
                if adapter_type not in first_last_key_frames:
                    continue

                key_prev = first_last_key_frames[adapter_type]['last']
                key_next = first_last_key_frames[adapter_type]['first']

                for p in sorted(embeddings_map.keys()):
                    if p > center_frame:
                        key_next = p
                        break
                    key_prev = p
                ...
                if key_prev == key_next or dist_prev + dist_next == 0:
                    embed = embeddings_map[key_prev]
                else:
                    rate = dist_prev / (dist_prev + dist_next)
                    embed = get_tensor_interpolation_method()(embeddings_map[key_prev], embeddings_map[key_next], rate)
                all_embeddings.append(embed)

            # Concatenate embeddings from all enabled adapters
            if all_embeddings:
        ...
        def get_current_prompt_embeds(
            ...
            if is_single_prompt_mode:
                return get_current_prompt_embeds_single(context,video_length)
            return get_current_prompt_embeds_multi(context,video_length)
